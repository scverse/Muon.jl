...
@testset "create mudata" begin
    ...
    ad1.var[!, :commoncol] = ["ad1_$i" for i ∈ 1:size(ad1, 2)]
    ad1.var_names = ["ad1_$i" for i ∈ 1:size(ad1, 2)]
    ad2.var[!, :commoncol] = ["ad2_$i" for i ∈ 1:size(ad2, 2)]
    ad2.var_names = ["ad2_$i" for i ∈ 1:size(ad2, 2)]
...
@testset "insert new AnnData" begin
    ad2.var[!, :testcol] = ["ad2_$i" for i ∈ 1:size(ad2, 2)]
    update!(md)
...
@testset "obs_var" begin
    ...
    @testset "$backend" for (readfun, writefun, backend) ∈
        ...
        @testset "obs_global_columns" begin
            ...
            update!(md)
            @test sort(names(md.obs)) == sort(vcat(["$m:demo" for m ∈ keys(md.mod)], ["demo"]))
            writefun(file, md)
            read_md = readfun(file, backed=false)
            @test sort(names(read_md.obs)) == sort(vcat(["$m:demo" for m ∈ keys(md.mod)], ["demo"]))
        end
