...
abstract type AbstractIndex2{T, V} <: AbstractVector{T} end
mutable struct Index2{T, V} <: AbstractIndex2{T, V}
    vals::Vector{T}
    indices::Vector{V}
...
function _getindex_array(idx::Index2{T, V}, elem::T) where {T, V}
    locations = Vector{V}()
    ilength = _length(idx)
    if ilength > 0x0
        elemhash = hash(elem)
...
function _getindex_byposition(idx::Index2{T}, i::Integer) where {T}
    elem = idx.vals[i]
    ilength = _length(idx)
    if ilength > 0x0
...
function _delete!(idx::Index2, oldkeyindex::Integer)
...
Base.getindex(idx::AbstractIndex2{T}, elem::T) where {T} = getindex(idx, elem, Val(false))
Base.getindex(idx::AbstractIndex2{T}, elem::T, x::Bool) where {T} = getindex(idx, elem, Val(x))
Base.getindex(idx::AbstractIndex2{T}, elem::T, x::Bool, y::Bool) where {T} = getindex(idx, elem, Val(x), Val(y))
Base.getindex(idx::AbstractIndex2{T}, elems::AbstractVector{T}) where {T} = getindex(idx, elems, Val(false))
Base.getindex(idx::AbstractIndex2{T}, elems::AbstractVector{T}, x::Bool) where {T} = getindex(idx, elems, Val(x))
Base.getindex(idx::AbstractIndex2{T, V}, elems::AbstractVector{T}, x::Union{Val{true}, Val{false}}) where {T, V} =
    isempty(elems) ? V[] : reduce(vcat, (getindex(idx, elem, x) for elem ∈ elems))
Base.in(elem::T, idx::AbstractIndex2{T}) where {T} = getindex(idx, elem, Val(false), Val(false)) != 0x0

Base.getindex(idx::Index2{T}, elem::T, ::Val{true}, ::Val{false}) where {T} =
    @inbounds idx.indices[_getindex_array(idx, elem)] # exceptions may be undesirable in high-performance scenarios
function Base.getindex(idx::Index2{T}, elem::T, ::Val{true}) where {T}
...
function Base.getindex(idx::Index2{T, V}, elem::T, ::Val{false}, ::Val{false}) where {T, V}
    i = _getindex(idx, elem)
    return i == 0x0 ? V(0x0) : @inbounds idx.indices[i]
end
function Base.getindex(idx::Index2{T}, elem::T, ::Val{false}) where {T}
...
Base.@propagate_inbounds function Base.getindex(idx::Index2, i::Integer)
    return idx.vals[i]
end

@inline function Base.setindex!(idx::Index2{T}, newval::T, i::Integer) where {T}
...
Base.values(idx::Index2) = idx.vals
Base.convert(::Type{<:Index2}, x::AbstractArray) = Index2(x)
Base.convert(::Type{<:Index2}, x::Index2) = x

struct SubIndex2{T, V, I} <: AbstractIndex2{T, V}
    parent::Index2{T, V}
    indices::I
    revmapping::Union{Nothing, Index2}
end
SubIndex2(idx::Index2{T, V}, indices::I) where {T, V, I} = SubIndex2{T, V, I}(idx, indices, nothing)

@inline function Base.view(idx::Index2, I::Union{AbstractRange, Colon})
    @boundscheck checkbounds(idx, I)
    return SubIndex2(idx, I)
end
@inline function Base.view(idx::Index2{T, V}, I::AbstractArray{<:Integer}) where {T, V}
    @boundscheck checkbounds(idx, I)
    return SubIndex2(idx, I, Index2(V.(I)))
end
@inline function Base.view(idx::Index2{T, V}, I::AbstractArray{Bool}) where {T, V}
    @boundscheck checkbounds(idx, I)
    return SubIndex2(idx, I, Index2(V.(findall(I))))
end
@inline function Base.view(idx::Index2, I::Integer)
    @boundscheck checkbounds(idx, I)
    return @inbounds view(idx, I:I)
end
@inline function Base.view(idx::SubIndex2, I)
    @boundscheck checkbounds(idx, I)
    return @inbounds view(parent(idx), Base.reindex((parentindices(idx),), (I,))[1])
end

Base.copy(si::SubIndex2) = Index2(si)
Base.parent(si::SubIndex2) = si.parent
Base.parentindices(si::SubIndex2) = si.indices
Base.length(si::SubIndex2) = length(parentindices(si))
Base.length(si::SubIndex2{T, V, Colon}) where {T, V} = length(parent(si))
Base.length(si::SubIndex2{T, V, I}) where {T, V, I <: AbstractArray{Bool}} = length(si.revmapping)
Base.size(si::SubIndex2) = (length(si),)
Base.values(si::SubIndex2) = parent(si)[parentindices(si)]
function Base.getindex(si::SubIndex2{T}, elem::T, ::Val{true}, ::Val{false}) where {T}
...
Base.getindex(si::SubIndex2{T, V, Colon}, elem::T, ::Val{true}, ::Val{false}) where {T, V} =
    getindex(parent(si), elem, Val(true), Val(false))
function Base.getindex(si::SubIndex2{T, V, I}, elem::T, ::Val{true}, ::Val{false}) where {T, V, I <: AbstractArray{Bool}}
    res = getindex(parent(si), elem, Val(true), Val(false))
...
function Base.getindex(si::SubIndex2{T, V, I}, elem::T, ::Val{true}, ::Val{false}) where {T, V, I <: AbstractRange}
    res = getindex(parent(si), elem, Val(true), Val(false))
    j = 0
    for i ∈ 1:length(res)
...
function Base.getindex(si::SubIndex2{T}, elem::T, ::Val{true}) where {T}
...
function Base.getindex(si::SubIndex2{T, V}, elem::T, ::Val{false}, ::Val{false}) where {T, V}
...
Base.getindex(si::SubIndex2{T, V, Colon}, elem::T, ::Val{false}, ::Val{false}) where {T, V} =
...
function Base.getindex(si::SubIndex2{T, V, I}, elem::T, ::Val{false}, ::Val{false}) where {T, V, I <: AbstractRange}
    res = getindex(parent(si), elem, Val(false), Val(false))
    if res > 0x0
...
function Base.getindex(si::SubIndex2{T}, elem::T, ::Val{false}) where {T}
...
Base.@propagate_inbounds function Base.getindex(si::SubIndex2, i::Union{Integer, AbstractVector{<:Integer}})
    @boundscheck checkbounds(si, i)
    return parent(si)[Base.reindex((parentindices(si),), (i,))[1]]
end
Base.@propagate_inbounds function Base.getindex(
    si::SubIndex2{T, V, Colon},
...
Base.@propagate_inbounds function Base.setindex!(si::SubIndex2{T}, newval::T, i::Integer) where {T}
    @boundscheck checkbounds(si, i)
    setindex!(parent(si), newval, Base.reindex((parentindices(si),), (i,))[1])
    return si
end
Base.@propagate_inbounds function Base.setindex!(si::SubIndex2{T, V, Colon}, newval::T, i::Integer) where {T, V}
    @boundscheck checkbounds(si, i)
    setindex!(parent(si), newval, i)
end
Base.@propagate_inbounds function Base.setindex!(si::SubIndex2{T}, newval::T, oldval::T) where {T}
