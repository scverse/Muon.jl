...
abstract type AbstractIndex5{T, V} <: AbstractVector{T} end
mutable struct Index5{T, V} <: AbstractIndex5{T, V}
    vals::Vector{T}
    indices::Vector{V}
    ...
    function Index5{T}(nelements::Integer) where {T}
...
_length(idx::Index5) = length(idx.indices)

function _setindex!(idx::Index5{T, V}, elem::T, position::Unsigned) where {T, V}
    idx.vals[position] = elem
    probeposition::V = 0x0
    k = position
    elemhash = celemhash = hash(elem)
    location = elemhash % _length(idx) + 0x1
...
function _getindex(idx::Index5{T, V}, elem::T) where {T, V}
    ilength = _length(idx)
    if ilength > 0x0
        elemhash = hash(elem)
        location::V = elemhash % ilength + 0x1
...
function _getindex_array(idx::Index5{T, V}, elem::T) where {T, V}
    if allunique(idx)
...
function _getindex_byposition(idx::Index5{T, V}, i::Integer) where {T, V}
    elem = idx.vals[i]
    ilength = _length(idx)
    if ilength > 0x0
...
function _delete!(idx::Index5{T, V}, oldkeyindex::V, elem::T) where {T, V}
...
function Index5(elems::AbstractVector{T}) where {T}
    idx = Index5{T}(length(elems))
...
Base.getindex(idx::AbstractIndex5{T}, elem::T) where {T} = getindex(idx, elem, Val(false))
Base.getindex(idx::AbstractIndex5{T}, elem::T, x::Bool) where {T} = getindex(idx, elem, Val(x))
Base.getindex(idx::AbstractIndex5{T}, elem::T, x::Bool, y::Bool) where {T} = getindex(idx, elem, Val(x), Val(y))
Base.getindex(idx::AbstractIndex5{T}, elems::AbstractVector{T}) where {T} = getindex(idx, elems, Val(false))
Base.getindex(idx::AbstractIndex5{T}, elems::AbstractVector{T}, x::Bool) where {T} = getindex(idx, elems, Val(x))
Base.getindex(idx::AbstractIndex5{T, V}, elems::AbstractVector{T}, x::Union{Val{true}, Val{false}}) where {T, V} =
    isempty(elems) ? V[] : reduce(vcat, (getindex(idx, elem, x) for elem ∈ elems))
Base.in(elem::T, idx::AbstractIndex5{T}) where {T} = getindex(idx, elem, Val(false), Val(false)) != 0x0

@inline Base.allunique(idx::Index5) = idx.duplicates == 0x0
Base.getindex(idx::Index5{T}, elem::T, ::Val{true}, ::Val{false}) where {T} =
    @inbounds idx.indices[_getindex_array(idx, elem)] # exceptions may be undesirable in high-performance scenarios
function Base.getindex(idx::Index5{T}, elem::T, ::Val{true}) where {T}
...
function Base.getindex(idx::Index5{T, V}, elem::T, ::Val{false}, ::Val{false}) where {T, V}
    i = _getindex(idx, elem)
    return i == 0x0 ? V(0x0) : @inbounds idx.indices[i]
end
function Base.getindex(idx::Index5{T}, elem::T, ::Val{false}) where {T}
...
Base.@propagate_inbounds function Base.getindex(idx::Index5, i::Integer)
    return idx.vals[i]
end

@inline function Base.setindex!(idx::Index5{T}, newval::T, i::Integer) where {T}
...
function Base.setindex!(idx::Index5{T}, newval::T, oldval::T) where {T}
...
Base.length(idx::Index5) = length(idx.vals)
Base.size(idx::Index5) = (length(idx),)
Base.values(idx::Index5) = idx.vals
Base.convert(::Type{<:Index5}, x::AbstractArray) = Index5(x)
Base.convert(::Type{<:Index5}, x::Index5) = x

struct SubIndex5{T, V, I} <: AbstractIndex5{T, V}
    parent::Index5{T, V}
    indices::I
    revmapping::Union{Nothing, Index5}
end
SubIndex5(idx::Index5{T, V}, indices::I) where {T, V, I} = SubIndex5{T, V, I}(idx, indices, nothing)

@inline function Base.view(idx::Index5, I::Union{AbstractRange, Colon})
    @boundscheck checkbounds(idx, I)
    return SubIndex5(idx, I)
end
@inline function Base.view(idx::Index5{T, V}, I::AbstractArray{<:Integer}) where {T, V}
    @boundscheck checkbounds(idx, I)
    return SubIndex5(idx, I, Index5(V.(I)))
end
@inline function Base.view(idx::Index5{T, V}, I::AbstractArray{Bool}) where {T, V}
    @boundscheck checkbounds(idx, I)
    return SubIndex5(idx, I, Index5(V.(findall(I))))
end
@inline function Base.view(idx::Index5, I::Integer)
    @boundscheck checkbounds(idx, I)
    return @inbounds view(idx, I:I)
end
@inline function Base.view(idx::SubIndex5, I)
    @boundscheck checkbounds(idx, I)
    return @inbounds view(parent(idx), Base.reindex((parentindices(idx),), (I,))[1])
end

Base.copy(si::SubIndex5) = Index5(si)
Base.parent(si::SubIndex5) = si.parent
Base.parentindices(si::SubIndex5) = si.indices
Base.length(si::SubIndex5) = length(parentindices(si))
Base.length(si::SubIndex5{T, V, Colon}) where {T, V} = length(parent(si))
Base.length(si::SubIndex5{T, V, I}) where {T, V, I <: AbstractArray{Bool}} = length(si.revmapping)
Base.size(si::SubIndex5) = (length(si),)
Base.values(si::SubIndex5) = parent(si)[parentindices(si)]
function Base.getindex(si::SubIndex5{T}, elem::T, ::Val{true}, ::Val{false}) where {T}
...
Base.getindex(si::SubIndex5{T, V, Colon}, elem::T, ::Val{true}, ::Val{false}) where {T, V} =
    getindex(parent(si), elem, Val(true), Val(false))
function Base.getindex(si::SubIndex5{T, V, I}, elem::T, ::Val{true}, ::Val{false}) where {T, V, I <: AbstractArray{Bool}}
    res = getindex(parent(si), elem, Val(true), Val(false))
...
function Base.getindex(si::SubIndex5{T, V, I}, elem::T, ::Val{true}, ::Val{false}) where {T, V, I <: AbstractRange}
    res = getindex(parent(si), elem, Val(true), Val(false))
    j = 0
    for i ∈ 1:length(res)
...
function Base.getindex(si::SubIndex5{T}, elem::T, ::Val{true}) where {T}
...
function Base.getindex(si::SubIndex5{T, V}, elem::T, ::Val{false}, ::Val{false}) where {T, V}
...
Base.getindex(si::SubIndex5{T, V, Colon}, elem::T, ::Val{false}, ::Val{false}) where {T, V} =
...
function Base.getindex(si::SubIndex5{T, V, I}, elem::T, ::Val{false}, ::Val{false}) where {T, V, I <: AbstractRange}
    res = getindex(parent(si), elem, Val(false), Val(false))
    if res > 0x0
...
function Base.getindex(si::SubIndex5{T}, elem::T, ::Val{false}) where {T}
...
Base.@propagate_inbounds function Base.getindex(si::SubIndex5, i::Union{Integer, AbstractVector{<:Integer}})
    @boundscheck checkbounds(si, i)
    return parent(si)[Base.reindex((parentindices(si),), (i,))[1]]
end
Base.@propagate_inbounds function Base.getindex(
    si::SubIndex5{T, V, Colon},
...
Base.@propagate_inbounds function Base.setindex!(si::SubIndex5{T}, newval::T, i::Integer) where {T}
    @boundscheck checkbounds(si, i)
    setindex!(parent(si), newval, Base.reindex((parentindices(si),), (i,))[1])
    return si
end
Base.@propagate_inbounds function Base.setindex!(si::SubIndex5{T, V, Colon}, newval::T, i::Integer) where {T, V}
    @boundscheck checkbounds(si, i)
    setindex!(parent(si), newval, i)
end
Base.@propagate_inbounds function Base.setindex!(si::SubIndex5{T}, newval::T, oldval::T) where {T}
