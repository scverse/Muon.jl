...
mutable struct Index4{T, V} <: AbstractVector{T}
    vals::Vector{T}
    indices::Vector{V}
...
function Base.getindex(idx::Index4{T}, elem::T) where T
...
function Base.getindex(idx::Index4, i::Integer)
    return idx.vals[i]
end

function Base.setindex!(idx::Index4{T}, elem::T, i::Integer) where T
